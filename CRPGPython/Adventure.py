...
mage = Player(10, 3, 1, 5, 20, 0, 0, 1)
rouge = Player(10, 10, 1, 0, 0, 0, 0, 1)
...
tornFabric = Item("Piece of Torn Fabric", "Pieces of Tor Fabric", 1, 0)
...
#Locations
class Location:
    def __init__(self, name, description, enemy, locToNorth, locToSouth, locToEast, locToWest, ogEnemy):
        self.name = name
        self.description = description
        self.enemy = enemy
        self.ogEnemy = ogEnemy
        
darkForest = Location("Dark Forest", "A dense forest packed with tall evergreens on all sides. Looking up, one can barely see the sun through the thick needles.", spider, None, None, None, None, spider)
forestPath = Location("Forest Path", "A path covered in stones, grass, and dirt that leads into the deep forest.", spider, None, None, None, None, spider)
home = Location("Home", "This is your home. It's quite barren.", null, None, None, None, None, null)
...
def Game():
    ...
    while character[0].health > 0:
        ...
        if playerInput == "help" or playerInput == "h":
            ...
            for x in inventory:
                if x.quantity == 1:
                    print(x.inventoryQuantity, x.name)
                else:
                    print(x.inventoryQuantity, x.pluralName)
            ...
            if currentLocation.locToNorth != None:
            ...
            if attackChoice == currentLocation.enemy.name and attackChoice != "null":
                global enemy
    ...
    while enemy.health > 0:
        ...
        if choice == "1":
            ...
            if hitChance >= 0:
                ...
                if enemy.health > 0:
                    ...
                    if character[0].experiencePoints >= 100:
                        ...
                        if lootChance >= 0:
                            ...
                            if enemy.loot.inventoryQuantity == 1:
                                print("You found", enemy.loot.inventoryQuantity , enemy.loot.name)
                            else:
                                print("You found", enemy.loot.inventoryQuantity , enemy.loot.pluralName)
                ...
                print("You now have", character[0].health, "health remaining.")
                GameOver(character)
        elif choice == "2":
            if character[0] != mage:
                ...
                if character[0].mana == 0:
                    ...
                    print("You try to conjur a magical blast to attack the", enemy.name)
                    hitChance = random.randint(0, 10)
                    if hitChance > 3:
                        enemy.health = enemy.health - character[0].mana
